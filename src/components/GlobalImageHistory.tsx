...
function FanItem({
  ...
  return (
    <button
      ...
      className="absolute w-14 h-14 overflow-hidden border-2 border-neutral-600 hover:border-blue-500 shadow-lg cursor-grab active:cursor-grabbing transition-colors duration-150 animate-fan-enter group"
      style={
        {
          "--fan-x": `${x}px`,
          "--fan-y": `${y}px`,
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-neutral-700 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="w-5 h-5 hover:bg-neutral-700 flex items-center justify-center text-neutral-400 hover:text-white transition-colors"
            title="Close"
          >
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
        {history.map((item, index) => (
          <div
            ...
            draggable
            onDragStart={(e) => onDragStart(e, item)}
            className="flex gap-3 p-2 hover:bg-neutral-700/50 cursor-grab active:cursor-grabbing group transition-colors"
          >
            {/* Thumbnail */}
            <div className="w-14 h-14 overflow-hidden shrink-0 border border-neutral-600 group-hover:border-blue-500 transition-colors">
              <img
                src={item.image}
                alt={`History ${index + 1}`}
                className="w-full h-full object-cover pointer-events-none"
          ...
          {hasOverflow && (() => {
            ...
            return (
              <button
                ...
                className="absolute animate-fan-enter bg-neutral-800 hover:bg-neutral-700 border border-neutral-600 px-2 py-1 text-[10px] text-neutral-300 hover:text-white shadow-lg transition-colors whitespace-nowrap"
                style={
                  {
